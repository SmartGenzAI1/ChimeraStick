...
class ChimeraServerTestCase(unittest.TestCase):
    ...
    def test_file_management_security_limits(self):
        ...
        # 4. Delete file
        response = self.app.post('/delete/notes.txt', data={
            'csrf_token': 'token123'
        }, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_settings_route(self):
        ...
        response = self.app.post('/settings', data={'discord_webhook': 'https://discord.com/api/webhooks/123/456'})
        self.assertEqual(response.status_code, 400)
        ...
        with open(self.config_file) as f:
            cfg = json.load(f)
        self.assertEqual(cfg.get('discord_webhook'), 'https://discord.com/api/webhooks/999/888')
