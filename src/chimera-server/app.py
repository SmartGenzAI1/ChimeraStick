...
def load_config():
    ...
    return {"password_hash": None, "tunnel_url": None, "secret_key": None, "discord_webhook": None}
...
def get_disk_usage():
    target_path = DATA_DIR if os.path.ismount(DATA_DIR) or os.path.exists(DATA_DIR) else '/'
    try:
                    ...
                    try:
                        ...
                        payload = {
                            "embeds": [{
                                ...
                                "footer": {
                                    "text": "ChimeraStick Secure Gateway"
                                },
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if bcrypt.checkpw(password, stored_hash):
                ...
                return render_template('login.html', error='Invalid administrator password')
        except Exception:
            return render_template('login.html', error='Corrupt key configuration. Reinitialize.')
...
def setup():
    ...
    if request.method == 'POST':
        ...
        if os.name != 'nt':
            try:
                subprocess.run(['chpasswd'], input=f"chimera:{password}", text=True, check=True)
            except Exception as e:
